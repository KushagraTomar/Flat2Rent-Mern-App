...
export default function PhotoUploader({addedPhotos, onChange}) {
    ...
    // function to upload images in upload folder
    ...
    // function for uploading images from the device
    ...
    return (
        <div>
            ...
            {/* display added photos and add photos from device */}
            <div className="mt-2 gap-2 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
              {addedPhotos.length > 0 && addedPhotos.map(link => (
                <div className="h-32 flex relative" key={link}>
                  <img className="rounded-2xl w-full object-cover" src={'http://localhost:4000/uploads/'+link} alt="" />
                  
                  {/* call removePhoto function to remove photos */}
                  ...
                  <button onClick={ev => mainPhoto(ev,link)} className="absolute cursor-pointer bottom-1 left-1 text-white bg-black bg-opacity-50 rounded-2xl py-2 px-3">
                    {link === addedPhotos[0] && (
